...
@interface EntertainmentController ()
...
- (void)viewDidLoad {
    ...
    [self displayVideosEntertainment];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *imageUrl =  [[self.entertainmentArray objectAtIndex:indexPath.row]objectForKey:@"arr"];
...
-(void)displayVideosEntertainment{
    
    NSLog(@"displayVideosEntertainment called");
    //http://104.155.210.134/index.php?method=liveVideosCategory&page=1&category=entertainment

        NSError * error=nil;
        NSURLResponse * urlReponse=nil;
        
        NSString * urlStr=[NSString stringWithFormat:@"http://104.155.210.134/index.php?method=liveVideosCategory&page=1&category=music"];
    ...
    for (NSDictionary *dic in [response objectForKey:@"message"] ) {
        [ self.entertainmentArray addObject:@{@"arr":[dic objectForKey:@"video_name"]}];
    }
